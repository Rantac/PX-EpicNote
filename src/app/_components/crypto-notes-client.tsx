...
export default function CryptoNotesClient() {
  const [notes, setNotes] = useState<CryptoNote[]>([]);
  ...
  const fetchNotes = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: "Error", description: "Failed to fetch crypto notes.", variant: "destructive" });
      }
    } catch (error) {
  ...
  const onCreateSubmit = async (values: z.infer<typeof noteSchema>) => {
    try {
      ...
      if (response.ok) {
        toast({ title: "Success", description: "Crypto note added." });
        createForm.reset();
        fetchNotes();
      } else {
  ...
  const onEditSubmit = async (values: z.infer<typeof noteSchema>) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({ title: "Success", description: "Crypto note updated." });
  ...
  const handleDelete = async (noteId: string) => {
    ...
    try {
      const response = await fetch(`/api/crypto?id=${noteId}`, {
        method: 'DELETE',
      });
